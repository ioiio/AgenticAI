def MultiAgentFn2():
    ...
    from langchain_google_genai import ChatGoogleGenerativeAI # Using the recommended Gemini LLM
    from langgraph_supervisor import create_supervisor
    from langchain_core.messages import convert_to_messages, HumanMessage # Added for clarity

    load_dotenv()

    # --- 🌟 GLOBAL CONFIGURATION 🌟 ---
    # 1. Read Tavily API Key from environment (must be set in .env or shell)
    TAVILY_API_KEY = os.getenv("TAVILY_API_KEY", "TAVILY_API_KEY")

    # 2. Define the full Tavily MCP Server configuration as a global constant
    TAVILY_MCP_SERVER_CONFIG = {
        "tavily-remote": {
            ...
            "args": [
                "-y",
                "mcp-remote",
                f"https://mcp.tavily.com/mcp/?tavilyApiKey={TAVILY_API_KEY}" # Use the global variable
            ],
            "transport": "stdio"
        }
    }

    # Define LLM models once
    ...
    async def run_agent(query):
        client = MultiServerMCPClient(TAVILY_MCP_SERVER_CONFIG)
        ...
        supervisor = create_supervisor(
            model=SUPERVISOR_LLM, # Using the global SUPERVISOR_LLM (gemini-2.5-pro)
